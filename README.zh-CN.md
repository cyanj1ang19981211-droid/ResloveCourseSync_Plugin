...
| `overlay.html` / `overlay.py` | 暗色悬浮窗 + Edge app 启动器（图钉置顶 / 自动纠正尺寸）；用**独立 Edge 档案**，退出按进程树精确收尾，不与用户自己的浏览器冲突 |
| `xlsx_to_json.py` | **把课件 xlsx 转成插件 JSON**（命令行入口） |
| `convert_course.py` / `convert.bat` | 图形化转换：弹文件选择框（支持多选）→ 生成 JSON |
| `launcher.py` | `start.bat` 背后真正干活的：启动前环境自检 + 清掉旧的自己人实例 + **端口被占自动换空闲端口** + 后台起后端 + 开悬浮窗 + 跟随图钉保置顶 + 关窗按进程树收尾 |
| `version.py` | 版本号唯一来源（启动日志、启动失败弹窗、体检报告、`/diag` 都带版本号，便于一眼确认是不是旧版） |
...
- 万一 `config.json` 里那个端口正好被别的程序占着，插件**会自动改用其他空闲端口**
  （启动日志里会写明换了哪个），不影响使用；重复双击 `start.bat` 也只会保留一个实例。
...
- **建时间线时取消了「使用项目设置」，时间线名旁边出现一个小齿轮图标，插件就抓不到课件了**：
  旧版本的 bug，**已修**，重新下载最新版即可，不用改任何设置。
  达芬奇播放头报的是**绝对时间码**，而课件是从头开始记的；不勾「使用项目设置」时，
  时间线起点不一定是 `00:00:00:00`（常见是 `01:00:00:00`），旧版直接拿绝对时间码去
  查课件，两边差了一整个小时，于是永远匹配不上、一直显示「未同步」。
  新版会读出**这条时间线的起点**并做换算，悬浮窗底部状态栏会标「起点对齐」，
  `检查环境.bat` 也会打印「时间线起点」和换算后的「课件内位置」。

- **双击 `start.bat` 弹出「后端启动失败：端口被占用」**：旧版本的 bug，**已修**。
  以前插件使用固定端口，上一次退出时留下的 TIME_WAIT 连接会让新进程绑不上，
  重启电脑后也可能再次复现。现在启动时会**先探测端口到底有没有活人在用**：
  真被占用就自动换一个空闲端口（并告诉你换成了哪个），TIME_WAIT 不再被误判；
  同时退出时**按进程树**把后端一起收掉，不会再留下孤儿进程占着端口。
  → 重新下载最新版即可。

- **我平时自己也开着 Edge，会不会和插件打架？插件退出会不会把我的 Edge 也关了？**：
  都不会。悬浮窗用的是**一份独立的 Edge 档案**（放在 `.runtime\edge-profile`），
  和你日常浏览用的那套完全隔离 —— 你正在看的标签页不受任何影响；插件退出时，
  也只关它自己开的那一个窗口，并会核对「是不是自己人」才动手。
  （旧版按窗口标题关窗，理论上存在误伤风险，现已成为按进程与专属档案精确定位。）
...
- **状态栏显示「已同步 · 课件已结束」**：播放头已经越过课件的**总时长**，属于正常提示
  （可能你在检查收尾镜头）。把播放头拖回课件范围内就会恢复显示当前环节。
- **状态栏里出现「起点对齐」**：说明这条时间线的起点不是 `00:00:00:00`（就是你建时间线时
  没勾「使用项目设置」那条），插件已经自动把时间码换算成「相对课件起点的时间」了。
...
- **v0.7.0** —— **修掉「动不动就端口被占用 / 关不干净 / 自定义起点时间线抓不到课件」三件事。**
  ① **时间线起点对齐**：达芬奇播放头报的是绝对时间码，建时间线时若取消「使用项目设置」
  （时间线名旁会出现小齿轮图标），起点往往是 `01:00:00:00` 而不是 `00:00:00:00`，
  旧版拿绝对时间码查课件就永远匹配不上、前端一直「未同步」。新版读出**这条时间线自己的
  起点与帧率**并换算成「相对课件起点的秒」再匹配；同时识别丢帧时间码（`;` 写法），
  播放头超过课件总长会明确显示「课件已结束」而不是静默失配。
  ② **不再和用户自己的 Edge 打架**：悬浮窗改用**独立 Edge 档案**（`.runtime\edge-profile`）
  打开，与用户日常浏览器完全隔离；退出时不再按窗口标题关窗，而是按「进程树 + 自身专属
  档案」精确识别自己人，杜绝误杀用户标签页。
  ③ **不再报「端口被占用」**：以前固定端口 + 上次退出留下的 TIME_WAIT 会让新进程绑不上，
  重启电脑也可能复现；现在启动前**先探测端口上有没有活着的监听者**，有就自动换空闲端口
  并把新端口写进 `.runtime\server.json` 告知前端；同时新增**父进程看门狗**（launcher 一消失
  后端即自杀）、**子进程登记与退出清理**（不再留孤儿 worker 占端口）与**启动器 BrokenPipe 防护**。
  另新增 `version.py`：启动日志、失败弹窗、体检报告、`/diag` 接口都会带版本号，
  以后报问题先对版本号，一眼就能看出有没有重新下载 ZIP。
